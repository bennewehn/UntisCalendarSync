...
async function getCalEvents() {
  ...
  timetable.forEach((element) => {
    ...
    data = {
      ...
      description:
        `room: ${element.ro[0] ? element.ro[0].name : "unknown"} 
teacher: ${element.te[0] ? element.te[0].name : "unkown"}` +
        `(${element.te[0] ? element.te[0].longname : "unkown"})`,
    };
    if (
      !config.subjectBlacklist.includes(data.summary) &&
      element.code !== "cancelled"
    )
      events.push(data);
  });
  return events;
...
async function addEvents(events) {
  ...
  for (let i = 0; i < events.length; i++) {
    for (let j = 0; j < currEvents.length; j++) {
      if (
        events[i].start.dateTime >= new Date(currEvents[j].start.dateTime) &&
  ...
  for (const element of events) {
    ...
    await sleep(100); // do not exceed rate limit 
  }
}
